...
class ActiveRecordTest < Minitest::Test
  ...
  def test_postgresql
    ...
    found = false
    ar_spans.each do |span|
      if span[:data][:activerecord][:sql] == "INSERT INTO \"blocks\" (\"name\", \"color\", \"created_at\", \"updated_at\") VALUES ($?, $?, $?, $?) RETURNING \"id\""
        found = true
      end
    end
    assert found
    ...
    found = false
    ar_spans.each do |span|
      if span[:data][:activerecord][:sql] == "SELECT  \"blocks\".* FROM \"blocks\" WHERE \"blocks\".\"name\" = $? ORDER BY \"blocks\".\"id\" ASC LIMIT $?"
        found = true
      end
    end
    assert found
    ...
    found = false
    ar_spans.each do |span|
      if span[:data][:activerecord][:sql] == "DELETE FROM \"blocks\" WHERE \"blocks\".\"id\" = $?"
        found = true
      end
    end
    assert found
